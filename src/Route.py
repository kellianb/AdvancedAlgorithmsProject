...
class Route:
    ...
    def can_merge_with_time_windows(self, route_j: "Route") -> bool:
        ...
        current_time = self.cost() - self.customers[-1].cost_to(self.warehouse)
    ...
    def brute_force(self):
        ...
        permutations = [
            Route(self.warehouse, list(record))
            for record in itertools.permutations(self.customers)
        ]
    ...
    def print(self, name: str = "Route"):
        ...
        print(f"Total cost: {self.distance()}")
        ...
        for i in range(len(self.customers)):
            ...
            print(
                f"{'⌂ Customer ' + str(i + 1) + '/' + str(len(self.customers)) :<30} ID: {self.customers[i].id}"
            )
    ...
    def plot(
        ...
        # Plot customers
        ax.scatter(
            [c.x for c in self.customers],
            [c.y for c in self.customers],
            c="blue",
            s=80,
            label="Customers",
            marker="o",
        ...
        # Plot route path
        ax.plot(
            path_x, path_y, "g--", alpha=0.7, linewidth=1.5, label="Route", zorder=1
        )
        ...
        for loc in self.customers:
            ax.annotate(
                f"C{loc.id}", (loc.x, loc.y), xytext=(5, 5), textcoords="offset points"
            )
