...
class Location:
    ...
    def cost_to(self, other: "Location", current_cost: int = 0) -> float:
        ...
        current_cost += self.service
        return current_cost

    def find_reachable(
        self, others: list["Location"], current_cost: int = 0
    ) -> list["Location"]:
        ...
        return [
            loc
            for loc in deepcopy(others)
            if loc.due_date >= self.distance_to(loc) + current_cost
        ]

    def find_deliverable(
        self, others: list["Location"], capacity: int, current_cost: int = 0
    ) -> list["Location"]:
        ...
        return [
            loc
            for loc in self.find_reachable(others, current_cost)
            if loc.demand <= capacity
        ]

    def find_closest(
        self, others: list["Location"]
    ...
    def find_nearest_reachable(
        self, others: list["Location"], current_cost: int = 0
    ) -> tuple[Optional["Location"], float, list["Location"]]:
